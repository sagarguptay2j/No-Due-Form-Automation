...
$output .= '<table>
    ...
    if(isset($_POST["query"]))
    {
        ...
        $queryStatus = "SELECT `Roll Number`,`Date` FROM `nodue_status` WHERE `Lab`='$labname' AND `Status`='Approved' AND `Roll Number` LIKE '%".$search."%'";
    ...
    $queryStatus = "SELECT `Roll Number`,`Date` FROM `nodue_status` WHERE `Lab`='$labname' AND `Status`='Approved'";
    $resultStatus = mysqli_query($con, $queryStatus);
    $count = 0;
    while($data = $resultStatus->fetch_assoc())
    {
